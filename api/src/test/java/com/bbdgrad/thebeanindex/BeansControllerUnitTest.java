...
@Test
  public void testGetGDPofCountryInBeans_Success() throws Exception {
    ...
    Mockito.when(gdpRepository.findAllReferencesByYearYearAndCountryName(year, countryName)).thenReturn(Optional.of(mockGDP));

    ResponseEntity<BeanGDPDto> response = (ResponseEntity<BeanGDPDto>) controller.getGDPOfCountryInBeans(countryName, beanName, years);

    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.ACCEPTED);
    GDPDto expectedDto = GDPDto.toDto(mockGDP);
    assertThat(response.getBody().getGdpAmount()).isEqualTo(expectedDto.getGdpAmount().divide((new BigDecimal(959)), 2, RoundingMode.HALF_UP));
